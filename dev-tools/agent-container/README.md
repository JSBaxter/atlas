...
|------------------------------|---------------------------------------------------|------|-----|
| repo root                    | `/workspace/atlas`                   | rw   | The cell repo. |
| `~/.claude/projects/<repo>`  | `/home/agent/.claude/projects/-workspace-atlas` | rw   | Sessions + memory persist across container runs. |
...
|-----------------------------------------|---------------------------------------------------------|---------------|
| `atlas-agent-home`         | `/home/agent`                                           | The whole agent home dir. Includes `~/.claude/` (OAuth tokens, session metadata) AND `~/.claude.json` (Claude Code's user-config file at the home root — the first-run wizard re-prompts forever if this isn't persisted). |
| `atlas-agent-venv-queue`   | `/workspace/atlas/dev-tools/queue/.venv`   | uv project venv for the bundled queue |
...
```bash
docker volume rm atlas-agent-home
...
docker build -t atlas-agent:latest dev-tools/agent-container/
...
- The repo is at `/workspace/atlas`. `git status` works
  as expected.
...
```bash
AGENT_CONTAINER_IMAGE=atlas-agent:latest \
    dev-tools/agent-container/run.sh
```
