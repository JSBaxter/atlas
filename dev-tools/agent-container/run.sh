#!/usr/bin/env bash
# Launch the atlas agent container with the right bind-mounts.
#
...
IMAGE="${AGENT_CONTAINER_IMAGE:-atlas-agent:latest}"
...
# /workspace/atlas, so the in-container project dir is
# named `-workspace-atlas`. Bind-mount the host's
...
CONTAINER_PROJECT_NAME="-workspace-atlas"
...
# already-built venv).
VENV_VOLUMES=(
    -v atlas-agent-venv-queue:/workspace/atlas/dev-tools/queue/.venv
)
...
if [[ -f "$REPO_ROOT/.claude/settings.local.json" ]]; then
    HOST_SETTINGS_MASK+=(-v /dev/null:/workspace/atlas/.claude/settings.local.json:ro)
fi
...
exec docker run --rm "${TTY_FLAGS[@]}" \
    ...
    -v "$REPO_ROOT:/workspace/atlas" \
    "${HOST_SETTINGS_MASK[@]}" \
    "${VENV_VOLUMES[@]}" \
    -v atlas-agent-home:/home/agent \
